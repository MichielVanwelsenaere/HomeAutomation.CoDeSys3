...
- initDMX: configures the dimmer with DMX configuration. For more info about Art-Net and DMX, [read this](./../AdditionalFunctionality/DMX_artnet.md).
  ...
  - `pDmxValues`: datatype _POINTER TO oscat_network.NETWORK_BUFFER_SHORT_, pointer to a global buffer. There is currently only one buffer, and thus one universe.
  - `dmxUniverse`: integer value of the universe. Metadata for MQTT only.
