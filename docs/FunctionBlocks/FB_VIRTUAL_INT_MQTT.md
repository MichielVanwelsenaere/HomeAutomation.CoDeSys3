...
|:--|:--|:--|
| `IN` | INT | Input for the value that should be published through MQTT, provision this input when using the virtual function block in output mode. |

**Outputs**

| Pin | Type | Description |
|:--|:--|:--|
| `OUT` | INT | Output for the value that is received through the MQTT subscription. provision this output in other processing logic when using the virtual function block in input mode. |
...
|:--|:--|:--|:--|
| `DefaultValue` | INT |  | Value to set at startup if default value at startup behavior is configured. |
...
| Parameter | Type | Default | Description |
|:--|:--|:--|:--|
| `Value` | INT |  | Value to publish. Only effective in output mode. |
<!-- fb-interface:end -->
...
```
The MQTT publish topic in this code example will be `Devices/PLC/Lab/Out/Virtuals/FB_VIRTUAL_INT_001` (MQTTPubSwitchPrefix variable + function block name). The subscription topic will be `Devices/PLC/Lab/In/Virtuals/FB_VIRTUAL_INT_001` (MQTTSubSwitchPrefix variable + function block name).
...
mqtt:
  ...
  - name: "FB_VIRTUAL_INT_001"
    state_topic: "Devices/PLC/Lab/Out/Virtuals/FB_VIRTUAL_INT_001"
    qos: 2  
...
- id: fb_virtual_int_001-to-mqtt
  ...
  action:
    ...
    data_template:
      topic: 'Devices/PLC/Lab/In/Virtuals/FB_VIRTUAL_INT_001'
      retain: true
